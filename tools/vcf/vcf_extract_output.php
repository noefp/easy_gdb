...
<?php
  //get input variables from the form vcf_extract_input.php
  ...
  // if the vcf_dir is not set, use the default vcf path
  ...
  else
  {
    ...
    echo '<div id="tables_container" class="row" style="display:none">';
  ...
  foreach ($lines_array as $vcf_line) {
    ...
    echo "<td class=\"acc_info\"><a class=\"acc_link\" name=\"snp_pos\" value=\"$snp_info[1]\"\>ACC Info</a></td>";
    echo "</tr>\n";
  }
  echo "</tbody></table>\n";
  }
...
#snp_table > tbody > tr:nth-child(odd) > .acc_info {
  background: #f2f2f2;
}

#snp_table > tbody > tr:nth-child(even) > .acc_info {
  background: #fff;
}
      ...
      jQuery.ajax({
        ...
        success: function (html_array) {
          ...
          if ($.fn.DataTable.isDataTable("#acc_table")) {
            // If the table already exists, clear it and destroy it
          ...
          datatable_with_subtable('#acc_table');
          // datatable('#acc_table','');
